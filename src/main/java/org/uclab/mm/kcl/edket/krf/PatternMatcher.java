...
public class PatternMatcher {
    ...
    public List<KRFRule> fireRule(Map<String, List<String>> conditionsValue, KRFKnowledgeBase knowledgeBase) {
        ...
        for (KRFRule rule : knowledgeBase.getRules()) {
            if (matchedRule(conditionsValue, rule.getConditionList())) {
                firedRules.add(rule);
            }
        }
        return firedRules;
    ...
    public boolean matchedRule(Map<String, List<String>> conditionsValue, List<KRFCondition> conditionsKeyList) {
        for (KRFCondition conditionsKey : conditionsKeyList) {
            String conditionKey = conditionsKey.getConditionKey();
    ...
    public boolean matchedCondition(List<String> conditionsValue, KRFCondition conditionsKey) {
        ...
        try {
            ...
            KRFConditionValueOperator cvo = KRFConditionValueOperator.getValueOperator(conditionsKey.getConditionValueOperator());
            
            switch (krfConditionType) {
                case String:
                    matched = TypeComparator.compare(conditionsValue.get(0), conditionsKey.getConditionValue(), cvo);
                break;
                case Integer:
                    matched = TypeComparator.compare(Integer.parseInt(conditionsValue.get(0)), Integer.parseInt(conditionsKey.getConditionValue()), cvo);
                break;
                case Boolean:
                    matched = TypeComparator.compare(conditionsValue.get(0).equalsIgnoreCase("yes"), conditionsKey.getConditionValue().equalsIgnoreCase("yes"), cvo);
                break;
                case Time:
                    int factTimeSec = KRFUtil.timeToSec(conditionsValue.get(0));
                    int ruleTimeSec = KRFUtil.timeToSec(conditionsKey.getConditionValue());
